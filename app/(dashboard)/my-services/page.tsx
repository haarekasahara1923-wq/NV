import { Package, ExternalLink, Calendar, Video, Share2, Activity, MessageSquare, MapPin } from 'lucide-react';
    ...
    <div className="space-y-6">
            ...
            {myServices.map((sub, i) => (
                <div key={i} className="bg-card rounded-2xl border p-6 hover:shadow-md transition-all group">
                    ...
                    <div className="flex gap-3">
                        <Link href={getAnalyticsLink(sub.service.slug)} className="flex-1">
                          <Button className="w-full text-xs">View Dashboard</Button>
                        </Link>
      ...
      <div className="bg-secondary rounded-2xl p-8 text-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { name: 'Meta Ads', price: '999' },
                { name: 'Google My Business', price: '399' },
                { name: 'ChatBot Installation', price: '499' }
              ].map((s, i) => (
                  <Link key={i} href="/services">
                    <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm border border-white/5 hover:bg-white/20 transition-colors cursor-pointer">
